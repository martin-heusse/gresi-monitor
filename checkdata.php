...
#First find the fist and last timestamps with non-null production in our time window
$timeWinStart=$now-$nb_hours_prod*3600;

$qr="select min(envelop.time) as tsi, max(envelop.time) as tsf from (select  (r.ts+m.timeoffset) as time, max(r.prod/m.peak_power) as maxprod from ".tp."readings r, ".tp."meters m  where r.ts>$timeWinStart and r.serial=m.serial group by time) as envelop where envelop.maxprod>0";
